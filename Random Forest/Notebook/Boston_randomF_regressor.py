...
print('Root Mean Squared Error:', np.sqrt(metrics.mean_squared_error(y_test, new_pred)))




feature_imp2 = pd.Series(clf_important.feature_importances_,index=x.columns).sort_values(ascending=False)


sns.barplot(x=feature_imp2, y=feature_imp2.values.index)
# Add labels to your graph
plt.xlabel('Feature Importance Score')
plt.ylabel('Features')
plt.title("Visualizing Important Features")
